...
- 라벨 카드: 원형 프로그레스바 + 읽음/전체 통계

### AllArticlesScreen / LabelDetailScreen

두 화면 모두 동일한 다중선택 패턴 구현:
- `_isSelecting` + `Set<dynamic> _selectedKeys` (Hive `article.key` 사용)
- 탭 전환 시 `_selectedKeys.clear()`
- AppBar: 일반 모드(checklist 아이콘) / 선택 모드(개수 + 전체선택 체크박스 + 취소)
- Row: 선택 모드(Checkbox + 탭으로 토글) / 일반 모드(읽음토글 + open_in_new + delete_outline)
...
| `AppColors.neonGreen` | #39FF14 | (읽음 방향 힌트) |
| `AppColors.softCoral` | #FF6B6B | (나중에 방향 힌트) |
