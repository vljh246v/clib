...
> 작성일: 2026-04-28
> 적용 위치: App Store Connect → My Apps → Clib → App Information / 각 로케일 탭, Google Play Console → 스토어 등록정보
...
- [iPhone / App Store 작업 마스터 플랜](#iphone--app-store-작업-마스터-플랜)
- [iPhone / App Store 우선순위 요약](#iphone--app-store-우선순위-요약)
- [참고](#참고)
...
```
pocket,readlater,raindrop,bookmark,reading,save,article,유튜브,인스타,틱톡,블로그,라벨
```
의도: 영문 경쟁 앱명 + 검색어 + 한국 플랫폼명. Title/Subtitle 단어 일체 미포함. 99 bytes.
...
#### Keywords (100 bytes)
```
pocket,raindrop,matter,goodlinks,instapaper,article,reading,list,youtube,tiktok,reddit,blog,label
```
97 bytes. `instagram`은 제거해 100 bytes 제한 안으로 조정.
...
#### Keywords (100 bytes)
```
pocket,readlater,raindrop,bookmark,article,reading,youtube,tiktok,ブログ,ラベル,メモ,記事
```
99 bytes. `instagram`은 제거해 100 bytes 제한 안으로 조정.
...
#### Keywords (100 bytes)
```
pocket,readlater,raindrop,bookmark,reading,article,youtube,tiktok,博客,标签,笔记,文章
```
93 bytes. `instagram`은 제거해 100 bytes 제한 안으로 조정.
...
#### Keywords (100 bytes)
```
pocket,readlater,raindrop,bookmark,reading,article,youtube,tiktok,部落格,標籤,筆記,文章
```
96 bytes. `instagram`은 제거해 100 bytes 제한 안으로 조정.
...
#### Keywords (100 bytes)
```
pocket,readlater,raindrop,marcador,articulo,lectura,lista,youtube,tiktok,blog,etiqueta,nota
```
91 bytes. `instagram`은 제거해 100 bytes 제한 안으로 조정.
...
> 적용 위치: Google Play Console → 사용자 확보 → 스토어 등록정보 → 기본 스토어 등록정보
> 아직 언어를 추가하지 않았다면 기본값 `한국어 - ko-KR`만 먼저 입력한다. 영어/일본어 등은 Android 출시 후 데이터 보고 추가.
...
| 앱 이름 | 30자, 검색 인덱싱 O |
| 간단한 설명 | 80자, 검색 인덱싱 O |
| 자세한 설명 | 4,000자, 검색 인덱싱 O |
| Keywords | 없음 |
| Subtitle | 없음 |
| Promotional Text | 없음 |
| 출시명/출시노트 | 릴리스 관리용. 스토어 검색 최적화 필드 아님 |

### 앱 카테고리 / 태그

```
카테고리: 생산성
태그: 노트, 도서/참고자료, 생산성, 자기 계발
```

`도구`는 범위가 넓고 Clib의 핵심 사용 의도와 약해서 제외 권장.
...
14자. Android는 `나중에 읽기`까지 넣으면 제목이 길어져서, 검색 핵심인 `링크 저장`을 우선한다.

### 간단한 설명 (80자)

```
저장한 링크를 카드처럼 스와이프하고 라벨 알림으로 다시 읽어보세요
```

36자. `링크`, `스와이프`, `라벨 알림`, `읽기` 의도를 자연스럽게 포함.

### 자세한 설명 (4,000자)

```
유튜브, 인스타그램, 블로그… 저장만 하고 안 읽는 링크가 쌓였나요?

클립(Clib)은 흩어진 링크를 카드 한 장씩 스와이프로 읽게 만드는 지식 도서관입니다.
라벨별 알림이 잊은 링크를 다시 꺼내, 저장 습관을 읽는 습관으로 바꿉니다.

■ 핵심 기능
- 회원가입 없이 시작
- 공유 버튼으로 어디서든 링크 저장
- 카드 스와이프: 오른쪽은 읽음, 왼쪽은 나중에
- 라벨로 저장한 링크 정리
- 라벨별 알림으로 읽지 않은 링크 다시 확인
- 한 줄 메모로 핵심 기록
- 북마크로 다시 볼 링크 보관
- 선택적 클라우드 동기화

■ 이런 분께 추천
- 저장만 하고 읽지 않는 링크가 많은 분
- Pocket, Raindrop 같은 나중에 읽기 앱을 찾는 분
- 유튜브, 블로그, SNS 링크를 한곳에 모으고 싶은 분
- 짧게 끊어 읽는 습관을 만들고 싶은 분

■ 개인정보
계정 없이 사용할 수 있으며, 기본 데이터는 기기에 저장됩니다.
로그인은 선택이며, 여러 기기에서 동기화할 때만 사용됩니다.
...
- 앱 아이콘: 기존 아이콘 사용
- 그래픽 이미지: 1024 x 500 px. 추천 문구는 `저장한 링크를 다시 읽는 습관`
- 휴대전화 스크린샷: 최소 2장, 최대 8장. 첫 3장 우선순위:
  1. 저장만 한 링크, 이제 읽게 됩니다
  2. 공유 버튼 하나로 어디서든 저장
  3. 오른쪽은 읽음, 왼쪽은 나중에

### 출시명 / 출시노트

출시명과 출시노트는 릴리스 만들기 화면에 입력한다. 기본 스토어 등록정보와 별개다.

출시명:
```
1.1.2 Android initial release
```
...
- Store Listing Experiments는 Android 출시 후 6주 단위로 실행
- 첫 실험 후보: 앱 이름은 유지하고 간단한 설명만 A/B 테스트
- Play Console에서 노출수, 스토어 등록정보 방문자, 설치 전환율을 14일 단위로 비교

---

## iPhone / App Store 작업 마스터 플랜
...
- [ ] **2.1** 한국어 스크린샷 컨셉 5컷 적용
  1. 저장만 한 링크, 이제 읽게 됩니다
  2. 공유 버튼 하나로 어디서든 저장
  3. 오른쪽은 읽음, 왼쪽은 나중에
  4. 라벨별 알림으로 다시 꺼내기
  5. 북마크·메모·읽음 상태까지 한곳에
...
- [ ] **7.4** 6주마다 Product Page Optimization 또는 Custom Product Page 실험

---

## iPhone / App Store 우선순위 요약
